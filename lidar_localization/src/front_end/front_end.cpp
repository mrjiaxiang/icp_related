...
bool FrontEnd::InitWithConfig() {
    std::string config_file_path = WORK_SPACE_PATH + "/config/front_end/config.json";

    ifs_.open(config_file_path,std::ios::binary);
//    YAML::Node config_node = YAML::LoadFile(config_file_path);
    reader_.parse(ifs_,value_);
    InitDataPath(value_);
    InitRegistration(registration_ptr_, value_);
    InitFilter("local_map", local_map_filter_ptr_, value_);
    InitFilter("frame", frame_filter_ptr_, value_);
    InitFilter("display", display_filter_ptr_, value_);

    return true;
}

bool FrontEnd::InitParam(const Json::Value& config_node) {
    key_frame_distance_ = config_node["key_frame_distance"].asFloat();
    local_frame_num_ = config_node["local_frame_num"].asInt();

    return true;
}

bool FrontEnd::InitDataPath(const Json::Value& config_node) {
    data_path_ = config_node["data_path"].asString();
    if (data_path_ == "./") {
...
bool FrontEnd::InitRegistration(std::shared_ptr<RegistrationInterface>& registration_ptr, const Json::Value& config_node) {
    std::string registration_method = config_node["registration_method"].asString();
...
bool FrontEnd::InitFilter(std::string filter_user, std::shared_ptr<CloudFilterInterface>& filter_ptr, const Json::Value& config_node) {
    std::string filter_mothod = config_node[filter_user + "_filter"].asString();
