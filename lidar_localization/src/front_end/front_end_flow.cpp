...
FrontEndFlow::FrontEndFlow(ros::NodeHandle& nh) {
    std::string config_file_path = WORK_SPACE_PATH + "/config/dataset/config.json";
    LOG(INFO)<< "config_file_path : "<<config_file_path;
    ifs_.open(config_file_path,std::ios::binary);
    reader_.parse(ifs_,value_);

//    YAML::Node config_node = YAML::LoadFile(config_file_path);
    InitSubscribers(nh, value_["measurements"]);
...
bool FrontEndFlow::InitSubscribers(ros::NodeHandle& nh, const Json::Value& config_node) {
    //
    // init TF sub & pub:
    //
    lidar_to_imu_tf_sub_ptr_ = std::make_shared<TFListener>(
        nh, 
        config_node["imu"]["frame_id"].asString(), config_node["lidar"]["frame_id"].asString()
    );
    lidar_to_map_tf_pub_ptr_ = std::make_shared<TFBroadCaster>(
        "/map", config_node["lidar"]["frame_id"].asString()
    );
    ...
    cloud_sub_ptr_ = std::make_shared<CloudSubscriber>(
        nh, 
        config_node["lidar"]["topic_name"].asString(), config_node["lidar"]["queue_size"].asInt()
    );
    imu_sub_ptr_ = std::make_shared<IMUSubscriber>(
        nh, 
        config_node["imu"]["topic_name"].asString(), config_node["imu"]["queue_size"].asInt()
    );
    gnss_sub_ptr_ = std::make_shared<GNSSSubscriber>(
        nh, 
        config_node["gnss"]["topic_name"].asString(), config_node["gnss"]["queue_size"].asInt()
    );
