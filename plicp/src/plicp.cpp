...
PLICPMatcher::PLICPMatcher(void) {
    m_r          = 0.03;
    m_h          = 0.10;
    m_Iterations = 10;
...
//构造函数．
PLICPMatcher::PLICPMatcher(double _r, double _h, int _iter) {
    m_r          = _r;
    m_h          = _h;
    m_Iterations = _iter;
}
...
void PLICPMatcher::setIterations(int _iter) { m_Iterations = _iter; }

//去除非法数据
void PLICPMatcher::RemoveNANandINFData(std::vector<Eigen::Vector2d>& _input) {
    //去除非法数据．
...
void PLICPMatcher::setSourcePointCloud(std::vector<Eigen::Vector2d>& _source_pcloud) {
...
void PLICPMatcher::setSourcePointCloudNormals(std::vector<Eigen::Vector2d>& _normals) {
    m_sourcePtCloudNormals = _normals;
}

//本函数会自动重新生nearNormal成kd树．
void PLICPMatcher::setTargetPointCloud(std::vector<Eigen::Vector2d>& _target_pcloud) {
...
bool PLICPMatcher::ImplicitMLSFunction(Eigen::Vector2d x, double& height) {
...
/**
 * @brief PLICPMatcher::projSourcePtToSurface
 ...
 */
void PLICPMatcher::projSourcePtToSurface(std::vector<Eigen::Vector2d>& in_cloud,
...
bool PLICPMatcher::SolveMotionEstimationProblem(std::vector<Eigen::Vector2d>& source_cloud,
...
/**
 * @brief PLICPMatcher::ComputeNormal
 * 计算法向量
 * @param nearPoints    某个点周围的所有激光点
 * @return
 */
Eigen::Vector2d PLICPMatcher::ComputeNormal(std::vector<Eigen::Vector2d>& nearPoints) {
    Eigen::Vector2d normal;
...
//      << V * D * V.inverse() << endl;
/**
 * @brief PLICPMatcher::Match
 * 最终使用的ICP匹配函数．
 * @param finalResult
 * @param covariance
 * @return
 */
bool PLICPMatcher::Match(Eigen::Matrix3d& finalResult, Eigen::Matrix3d& covariance) {
    //如果没有设置法向量，则自动进行计算．
    ...
    for (int i = 0; i < m_Iterations; i++) {
        ...
        if (in_cloud.size() < 5 || ref_cloud.size() < 5) {
            std::cout << "Not Enough Correspondence: i " << i << ", " << in_cloud.size() << "," << ref_cloud.size()
                      << std::endl;
...
bool PLICPMatcher::SolverFourthOrderPolynomial(Eigen::VectorXd& p_coffi, double& lambda) {
