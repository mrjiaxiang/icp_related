...
class RTree {
 ...
 public:
    ...
    bool KNN(const ELEMTYPE coord[NUMDIMS], const int k_value, std::vector<DATATYPE>& result,
             ELEMTYPEREAL dis_limit = -1, t_resultCallback predicate = nullptr, void* a_context = nullptr);
 ...
 protected:
    ...
    struct KNNode {
        ...
        bool operator<(const KNNode& other) const { return m_distance < other.m_distance; }
...
RTREE_TEMPLATE
bool RTREE_QUAL::KNN(const ELEMTYPE coord[NUMDIMS], const int k_value, std::vector<DATATYPE>& result,
                     ELEMTYPEREAL dis_limit, t_resultCallback predicate, void* a_context) {
    ...
    while (node) {
        ...
        while (knnode_queue.size()) {
            ...
            if (knode.isLeaf()) {
                ...
                if (result.size() == (unsigned int)k_value)
                    return true;
            } else {
                break;
            }
